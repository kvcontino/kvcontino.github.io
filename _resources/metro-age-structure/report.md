...
### Corrections made to the specified sources

Five assumptions in the project specification did not survive contact with the files:
...
### 1M+ population (56 metros)

| metro | population | median age | prime-age median | college GQ | military GQ |
|---|---:|---:|---:|---:|---:|
| Salt Lake City-Murray, UT | 1,308,377 | 34.3 | 41.6 | 0.3% | 0.0% |
...
### 500k–1M population (55 metros)

| metro | population | median age | prime-age median | college GQ | military GQ |
|---|---:|---:|---:|---:|---:|
| Provo-Orem-Lehi, UT | 773,426 | 26.8 | 40.3 | 1.8% | 0.0% |
...
### 250k–500k population (85 metros)

| metro | population | median age | prime-age median | college GQ | military GQ |
|---|---:|---:|---:|---:|---:|
| College Station-Bryan, TX | 287,476 | 28.7 | 41.6 | 6.5% | 0.0% |
...
### 100k–250k population (166 metros)

| metro | population | median age | prime-age median | college GQ | military GQ |
|---|---:|---:|---:|---:|---:|
| Manhattan, KS | 136,122 | 27.5 | 39.0 | 4.5% | 3.7% |
...
### under 100k population (25 metros)

| metro | population | median age | prime-age median | college GQ | military GQ |
|---|---:|---:|---:|---:|---:|
| Hinesville, GA | 91,870 | 29.5 | 38.7 | 0.0% | 3.2% |
...
Six of the eight specified measures are computed. Two are omitted, and the reasons matter more than the omissions:

- **Prospective OADR — omitted.** It requires local life tables by age. The only county-level source identified, NCHS USALEEP, is a **2010–2015 vintage** and excludes Maine and Wisconsin outright. It was implemented, then removed: pairing decade-old life tables with Vintage 2025 population would put a stale denominator behind a current numerator, and the resulting threshold ages would have looked authoritative while resting on a decade-old mortality regime.
- **Economic support ratio — omitted.** It requires the National Transfer Accounts age profiles of labour income and consumption. NTA publishes these only through an interactive two-step selection interface with no stable machine-readable endpoint. Supplying the profile values from memory is precisely what this project forbids, so the measure is absent rather than fabricated.
...
**The third check failed for two metros, and the failure is a finding.** The specification expects Austin, Raleigh, Denver, Nashville, Charlotte, Salt Lake City–Murray, Boise, Washington DC and Seattle all to score in the top quintile. Salt Lake City–Murray reaches the 68th percentile and Washington the 45th.
...
| metro | spec weights | equal | CCR only | no domestic migration | migration-heavy |
|---|---:|---:|---:|---:|---:|
...
Washington never reaches the top quintile under **any** weighting. Salt Lake City reaches it only when domestic migration is removed entirely, which localises the cause precisely. The index is measuring domestic attraction and these metros did not have it over this window; the specification's expectation encodes a pre-2020 prior. The fourth exclusion criterion independently routes both into the international-led group, which is where the specification says such metros belong.
...
- **Prospective OADR and the economic support ratio are absent** (Section 4). The seven-measure comparison is therefore a six-measure comparison.
- **The PUMS household-population validation was not run**, so the DHC-based group-quarters subtraction has no independent check.
- **QWI counts jobs at the workplace, not the residence.** For the administrative support ratio this inflates the denominator in commuter-receiving metros and deflates it in commuter-sending ones.
- **SSA county counts are rounded** to the nearest 5 in small counties and are dated December 2024 against a 7/1/2025 population.
- **The seasonal flag uses 2 of 3 signals.** H-2A/H-2B certification data is published as per-application worksite addresses requiring geocoding, which was out of scope.
...
- **Silhouette scores for the typology are modest** (0.22 at best). The clusters describe the data; they are not evidence of discrete kinds.
...
387 rows × 126 columns.

| column | dtype | description |
|---|---|---|
| `cbsa` | str | 5-digit CBSA code, OMB July 2023 delineation. |
| `cbsa_title` | str | CBSA name as published in the Vintage 2025 files. |
| `lsad` | str | Legal/statistical area description (Metropolitan or Micropolitan Statistical Area). |
| `is_metro` | bool | True for a metropolitan statistical area, False for micropolitan. |
...
| `share_40_64` | float64 | Population aged 40–64 ÷ total population. |
| `share_65_74` | float64 | Population aged 65–74 ÷ total population. |
| `share_75p` | float64 | Population aged 75 and over ÷ total population. |
| `prime_age_median` | float64 | Median age within the 25–64 population. |
| `early_career_share` | float64 | Population 25–39 ÷ population 25–64. |
| `oadr` | float64 | Old-age dependency: population 65+ ÷ population 25–64. |
| `late_old_ratio` | float64 | Population 75+ ÷ population 25–64. |
| `replacement_ratio` | float64 | Population 20–24 ÷ population 60–64. |
| `share_under5` | float64 | Population under 5 ÷ total population. |
| `ccr_0_4__5_9` | float64 | Cohort change ratio: population aged 5–9 on 7/1/2025 ÷ population aged 0–4 on 7/1/2020. |
| `ccr_5_9__10_14` | float64 | Cohort change ratio: population aged 10–14 on 7/1/2025 ÷ population aged 5–9 on 7/1/2020. |
| `ccr_10_14__15_19` | float64 | Cohort change ratio: population aged 15–19 on 7/1/2025 ÷ population aged 10–14 on 7/1/2020. |
| `ccr_15_19__20_24` | float64 | Cohort change ratio: population aged 20–24 on 7/1/2025 ÷ population aged 15–19 on 7/1/2020. |
| `ccr_20_24__25_29` | float64 | Cohort change ratio: population aged 25–29 on 7/1/2025 ÷ population aged 20–24 on 7/1/2020. |
| `ccr_25_29__30_34` | float64 | Cohort change ratio: population aged 30–34 on 7/1/2025 ÷ population aged 25–29 on 7/1/2020. |
| `ccr_30_34__35_39` | float64 | Cohort change ratio: population aged 35–39 on 7/1/2025 ÷ population aged 30–34 on 7/1/2020. |
| `ccr_35_39__40_44` | float64 | Cohort change ratio: population aged 40–44 on 7/1/2025 ÷ population aged 35–39 on 7/1/2020. |
| `ccr_40_44__45_49` | float64 | Cohort change ratio: population aged 45–49 on 7/1/2025 ÷ population aged 40–44 on 7/1/2020. |
| `ccr_45_49__50_54` | float64 | Cohort change ratio: population aged 50–54 on 7/1/2025 ÷ population aged 45–49 on 7/1/2020. |
| `ccr_50_54__55_59` | float64 | Cohort change ratio: population aged 55–59 on 7/1/2025 ÷ population aged 50–54 on 7/1/2020. |
| `ccr_55_59__60_64` | float64 | Cohort change ratio: population aged 60–64 on 7/1/2025 ÷ population aged 55–59 on 7/1/2020. |
| `ccr_60_64__65_69` | float64 | Cohort change ratio: population aged 65–69 on 7/1/2025 ÷ population aged 60–64 on 7/1/2020. |
...
| `gq_correctional_18_64` | float64 | Group-quarters population of type 'correctional' in age group 18 64, scaled to 2025. |
| `gq_correctional_65p` | float64 | Group-quarters population of type 'correctional' in age group 65p, scaled to 2025. |
| `gq_juvenile_u18` | float64 | Group-quarters population of type 'juvenile' in age group u18, scaled to 2025. |
| `gq_juvenile_18_64` | float64 | Group-quarters population of type 'juvenile' in age group 18 64, scaled to 2025. |
| `gq_juvenile_65p` | float64 | Group-quarters population of type 'juvenile' in age group 65p, scaled to 2025. |
| `gq_nursing_u18` | float64 | Group-quarters population of type 'nursing' in age group u18, scaled to 2025. |
| `gq_nursing_18_64` | float64 | Group-quarters population of type 'nursing' in age group 18 64, scaled to 2025. |
| `gq_nursing_65p` | float64 | Group-quarters population of type 'nursing' in age group 65p, scaled to 2025. |
| `gq_other_inst_u18` | float64 | Group-quarters population of type 'other inst' in age group u18, scaled to 2025. |
| `gq_other_inst_18_64` | float64 | Group-quarters population of type 'other inst' in age group 18 64, scaled to 2025. |
| `gq_other_inst_65p` | float64 | Group-quarters population of type 'other inst' in age group 65p, scaled to 2025. |
| `gq_college_u18` | float64 | Group-quarters population of type 'college' in age group u18, scaled to 2025. |
| `gq_college_18_64` | float64 | Group-quarters population of type 'college' in age group 18 64, scaled to 2025. |
| `gq_college_65p` | float64 | Group-quarters population of type 'college' in age group 65p, scaled to 2025. |
| `gq_military_u18` | float64 | Group-quarters population of type 'military' in age group u18, scaled to 2025. |
| `gq_military_18_64` | float64 | Group-quarters population of type 'military' in age group 18 64, scaled to 2025. |
| `gq_military_65p` | float64 | Group-quarters population of type 'military' in age group 65p, scaled to 2025. |
| `gq_other_noninst_u18` | float64 | Group-quarters population of type 'other noninst' in age group u18, scaled to 2025. |
| `gq_other_noninst_18_64` | float64 | Group-quarters population of type 'other noninst' in age group 18 64, scaled to 2025. |
| `gq_other_noninst_65p` | float64 | Group-quarters population of type 'other noninst' in age group 65p, scaled to 2025. |
| `gq_all_u18` | float64 | Group-quarters population of type 'all' in age group u18, scaled to 2025. |
| `gq_all_18_64` | float64 | Group-quarters population of type 'all' in age group 18 64, scaled to 2025. |
| `gq_all_65p` | float64 | Group-quarters population of type 'all' in age group 65p, scaled to 2025. |
| `gq_correctional_total` | float64 | Group-quarters population of type 'correctional', all ages, scaled to 2025. |
| `gq_juvenile_total` | float64 | Group-quarters population of type 'juvenile', all ages, scaled to 2025. |
| `gq_nursing_total` | float64 | Group-quarters population of type 'nursing', all ages, scaled to 2025. |
| `gq_other_inst_total` | float64 | Group-quarters population of type 'other inst', all ages, scaled to 2025. |
...
| `enrolled_15_17` | int64 | ACS college/graduate enrollment, ages 15 17. |
| `enrolled_18_24` | int64 | ACS college/graduate enrollment, ages 18 24. |
| `enrolled_25_34` | int64 | ACS college/graduate enrollment, ages 25 34. |
| `enrolled_35p` | int64 | ACS college/graduate enrollment, ages 35p. |
| `enrolled_total` | int64 | ACS college/graduate enrollment, all ages. |
| `qwi_emp_25_64` | float64 | LEHD QWI employment aged 25–64, 2025Q1 (sum of bands A04–A07). |
| `qwi_emp_25_34_2025` | float64 | QWI employment aged 25–34, 2025Q1. |
| `qwi_emp_25_34_2020` | float64 | QWI employment aged 25–34, 2020Q1. |
| `retired_workers` | float64 | SSA retired-worker beneficiaries, December 2024. |
...
| `prime_age_median_hh` | float64 | Prime-age median on the household population. |
| `oadr_hh` | float64 | As above on the household population. |
| `early_career_share_hh` | float64 | As above on the household population. |
| `gq_total_share` | float64 | Group quarters ÷ total population. |
| `gq_correctional_share` | float64 | Group-quarters population of type 'correctional' ÷ total population. |
| `gq_juvenile_share` | float64 | Group-quarters population of type 'juvenile' ÷ total population. |
| `gq_nursing_share` | float64 | Group-quarters population of type 'nursing' ÷ total population. |
| `gq_other_inst_share` | float64 | Group-quarters population of type 'other inst' ÷ total population. |
...
| `gq_other_noninst_share` | float64 | Group-quarters population of type 'other noninst' ÷ total population. |
| `prime_age_median_student_adj` | float64 | Prime-age median after also removing ACS college-enrolled 25–34. |
| `prime_age_median_student_delta` | float64 | prime_age_median_hh minus prime_age_median_student_adj. NEGATIVE by construction; the magnitude is the college-town diagnostic. |
| `admin_support_ratio` | float64 | SSA retired-worker beneficiaries (Dec 2024) ÷ QWI employment aged 25–64 (2025Q1). Null where QWI does not cover every constituent county. |
...
| `mode_sharpness_18_24` | float64 | Mean population share at ages 18–24 ÷ mean share at 30–39. Near 1.0 with no institutional population. |
| `share_25_49` | float64 | Population 25–49 ÷ total population. |
| `intl_share_of_net` | float64 | International migration ÷ total net migration, 2020–2025, clipped to [0,1]; 1.0 where net migration is non-positive but international is positive. |
| `rank_ccr_prime_mean` | float64 | Percentile rank of ccr prime mean used as a magnet-index component. |
| `rank_early_career_share` | float64 | Percentile rank of early career share used as a magnet-index component. |
| `rank_net_domestic_mig_rate` | float64 | Percentile rank of net domestic mig rate used as a magnet-index component. |
| `rank_qwi_emp_25_34_growth` | float64 | Percentile rank of qwi emp 25 34 growth used as a magnet-index component. |
| `rank_oadr_inverted` | float64 | Percentile rank of oadr inverted used as a magnet-index component. |
| `magnet_index` | float64 | Weighted mean of percentile ranks of the five magnet components (0–1). |
| `magnet_weight_coverage` | float64 | Share of the total weight actually available for this metro. |
| `magnet_index_partial` | bool | True where a component was missing and weights were renormalised. |
| `magnet_rank` | int64 | Rank on magnet_index, 1 = highest. |
| `magnet_pctile` | float64 | Percentile rank on magnet_index. |
| `excl_gq_total` | bool | Exclusion criterion 'gq total' fired for this metro. |
| `excl_gq_college` | bool | Exclusion criterion 'gq college' fired for this metro. |
| `excl_high_fertility` | bool | Exclusion criterion 'high fertility' fired for this metro. |
| `excl_international` | bool | Exclusion criterion 'international' fired for this metro. |
| `excluded_any` | bool | True where at least one exclusion criterion fired. |
| `domestic_magnet_eligible` | bool | True where no exclusion criterion fired. |
| `international_led` | bool | True where only the international-migration criterion fired. |

### `measures_cbsa.parquet`

925 rows × 106 columns.
...
3,144 rows × 116 columns.

| column | dtype | description |
|---|---|---|
| `fips` | str | 5-digit county FIPS (state + county). |
| `state_fips` | str | 2-digit state FIPS. |
| `county_fips` | str | 3-digit county code within state. |
| `state_name` | str | State name. |
| `county_name` | str | County or county-equivalent name. |
| `total_pop` | float64 | Total resident population, 7/1/2025 estimate. |
| `total_pop_2020` | float64 | Total resident population, 7/1/2020 estimate (YEAR index 2). |
| `median_age` | float64 | Median age of all ages, linear interpolation within the median single year. |
| `dissimilarity` | float64 | Age-structure dissimilarity index, 0.5·Σ|p−q| over single-year shares against the nation. |
...
| `enrolled_35p` | int64 | ACS college/graduate enrollment, ages 35p. |
| `enrolled_total` | int64 | ACS college/graduate enrollment, all ages. |
| `acs_vintage` | int64 | ACS 5-year vintage actually used (2024, or 2023 on fallback). |
| `prime_age_median_student_adj` | float64 | Prime-age median after also removing ACS college-enrolled 25–34. |
| `prime_age_median_student_delta` | float64 | prime_age_median_hh minus prime_age_median_student_adj. NEGATIVE by construction; the magnitude is the college-town diagnostic. |
| `qwi_emp_25_64` | float64 | LEHD QWI employment aged 25–64, 2025Q1 (sum of bands A04–A07). |
| `qwi_emp_25_34_2025` | float64 | QWI employment aged 25–34, 2025Q1. |
...
| `qwi_emp_25_34_growth` | float64 | Ratio of the two preceding columns. |
| `retired_workers` | float64 | SSA retired-worker beneficiaries, December 2024. |
| `ssa_year` | int64 | SSA data year (2024). |
| `admin_support_ratio` | float64 | SSA retired-worker beneficiaries (Dec 2024) ÷ QWI employment aged 25–64 (2025Q1). Null where QWI does not cover every constituent county. |
| `admin_support_available` | bool | True where admin_support_ratio could be computed. |
| `seasonal_housing_share` | float64 | Seasonal/occasional-use housing units ÷ total housing units (ACS B25004_006 ÷ B25001_001). |
| `qwi_amplitude` | float64 | (max − min) ÷ mean of the four 2024 quarterly all-ages QWI employment values. |
| `top_decile_seasonal_housing_share` | bool | True where the county is in the top decile of seasonal housing share. |
| `top_decile_qwi_amplitude` | bool | True where the county is in the top decile of qwi amplitude. |
...
| `prime_age_median_county_p25` | float64 | Within-state prime age median percentile p25 across the state's counties. |
| `prime_age_median_county_p75` | float64 | Within-state prime age median percentile p75 across the state's counties. |
| `oadr_county_p25` | float64 | Within-state oadr percentile p25 across the state's counties. |
| `oadr_county_p75` | float64 | Within-state oadr percentile p75 across the state's counties. |
...
| `sex_ratio_18_29` | float64 | Males per 100 females aged 18–29. |
| `share_0_17` | float64 | Population aged 0–17 ÷ total population. |
| `share_65p` | float64 | Population 65+ ÷ total population. |
| `cluster_ward` | int64 | Ward hierarchical cluster label (k=6). |
| `cluster_kmeans` | int32 | k-means cluster label (k=6); the labels carried forward. |

### `youth_decomposition.parquet`

190 rows × 136 columns.
...
| `domestic_magnet_eligible` | bool | True where no exclusion criterion fired. |
| `international_led` | bool | True where only the international-migration criterion fired. |
| `score_fertility` | float64 | Raw cause score for 'fertility': mean of the percentile ranks of its diagnostics. |
| `score_institution` | float64 | Raw cause score for 'institution': mean of the percentile ranks of its diagnostics. |
| `score_immigration` | float64 | Raw cause score for 'immigration': mean of the percentile ranks of its diagnostics. |
| `score_attraction` | float64 | Raw cause score for 'attraction': mean of the percentile ranks of its diagnostics. |
| `contrib_fertility` | float64 | Share of the youth explanation attributed to 'fertility'; the four sum to 1. |
| `contrib_institution` | float64 | Share of the youth explanation attributed to 'institution'; the four sum to 1. |
| `contrib_immigration` | float64 | Share of the youth explanation attributed to 'immigration'; the four sum to 1. |
| `contrib_attraction` | float64 | Share of the youth explanation attributed to 'attraction'; the four sum to 1. |
| `dominant_cause` | str | Argmax of the four youth-cause scores. Only meaningful alongside dominant_share. |
| `dominant_share` | float64 | The largest of the four normalised contributions. Near 0.25 means the causes are tied. |
